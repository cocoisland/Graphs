...
def toEndNode(curRoom, direction):     
    while direction in graph[curRoom] :
        if graph[curRoom][direction] == '?':
            ...
            #print(f' inside === endNode  cur={curRoom} dir={direction} travese={traversalPath} graph={graph}')
        else:
            #print(f'endNode break cur={curRoom} dir={direction} travese={traversalPath} graph={graph}')
            break
...
while direction in graph[curRoom] and graph[curRoom][direction] == '?':
    curRoom= toEndNode(curRoom, direction)

    untraversed = [ k for k in graph[curRoom].keys() if graph[curRoom][k] == '?']

    #print(f' traver={traversalPath}')
    #print(f' cur={curRoom} dir={direction} untrav={untraversed} graph = {graph}')
    if len(untraversed) > 1 :
        for u in untraversed :
            if u != backDirection(direction) :
                curRoom = toEndNode(curRoom, u)
